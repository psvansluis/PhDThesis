...
% The phonetic stage of lenition whereby a single-geminate distinction was reanalysed as a short-long distinction must have preceded the stage when this distinction became phonemic, but Martinet is unsure how much earlier. Lenition may either be considered an early \acrlong{pc} development, or a later Pan-Celtic areal feature developing because of parallel development. Such a parallel development could be the result of a common substrate or spread from one dialect to another. He leans towards a Pan-Celtic scenario, because of the problem of different treatments of \lT\ in Goidelic and Brittonic where intervocalic \pc{-t-} ultimately yields [θ] in Goidelic, but [d] in Brittonic. It is difficult to infer [d] from [θ] or vice versa. Yet he then notes:
% \tqt{But this of course is not decisive: intervocalic \textit{t} may have been weakened in Proto-Celtic, let us say, to a voiceless media (a lenis stop) from which both [θ] and [d] developed at a later date\footnote{The term ‘voiceless media (a lenis stop)’ seems contradictory: ‘media’ is a dated synonym for a voiced stop consonant, yet the term is preceded by ‘voiceless’, its antonym. It seems that Martinet means partial or weak voicing here, a possibility which I reject in Section~\ref{sec:voice-aspiration}, because it would entail a three-way distinction in voice.}.}{martinet_celtic_1952}{195}
...
\begin{mwl}
  \langc[]{\cite[64]{falchun_systeme_1951}}{\mob{tôrrèd éó é \al{g}ār; tôrrèd éó é \al{gg}ār}\footnote{Note that this example is not technically a minimal pair, because the /r/ is long in \mob[car]{karr}, but short in \mob[leg]{gar}. However, this difference may be neutralised word-finally~\autocite[34]{carlyle_syllabic_1988}.}}{his car is broken; her leg is broken}
...
\tqt{
  \begin{french}
    ...
    Le \textit{b} sera doux dans \textit{abred, (abr\c{ē}d)}, «de bonne heure», mais fort dans \textit{ha breman (a bbr\`{\c{ē}}mã)}, «et à présent», parce que \textit{ha}, réduction de \textit{hag}, n'adoucit pas la consonne suivante […].
...
He then notes that radical \mob{n-, l-, r-} are long consonants and that they are identical to medial long \mob{nn, ll, rr} following the accent, while lenited \mob{n-, l-, r-} are identical to short \mob{n, l, r} following the accent. These long and short sonorants similarly form minimal pairs, so they are phonemically distinct:
\begin{mwl}
...
Apparently Jackson attempts to date the merger of \lT\ and \xD, because he writes about \pbr[]{t} becoming full \pbr[]{d}. Yet he also mentions the date when lenited allophones became separate \gls{phoneme}s as  relevant. I take the second criterion to mean the moment when lenited and radical allophones were sufficiently different that they could be reinterpreted as separate \gls{phoneme}s rather than the moment when radical and lenited consonants were actually separate \gls{phoneme}s. This is because when he sets out the evidence for his date, the first thing he notes is:
...
\begin{table}[h]
  ...
  \begin{tabular}{cccccccccccc}
    \toprule
    & \multicolumn{4}{c}{Initial sandhi} & \tchh{Abs.~initial} & \multicolumn{4}{c}{Internal intervocalic} & \tch{\gls{R}\gls{T}} \\
    \midrule
...
\begin{table}[h]
  ...
  \begin{tabular}{cccccccccccc}
    \toprule
    & \multicolumn{4}{c}{Initial sandhi} & \tchh{Abs.~initial} & \multicolumn{4}{c}{Internal intervocalic} & \tch{\gls{R}T} \\
    \midrule
...
The third argument is that the \gls{rescon}s in Le Bourg Blanc Breton provide a solid analogical base. This argument holds more water. Harvey notes that \mob{n, l, r} can be both long and short word-initially. Thus, length is the only feature keeping radical and lenited \gls{rescon}s apart word-initially, so length in stops may not be original, but the outcome of the analogical extension of the system:
\begin{center}
  \begin{tabular}{l@{~=~}l}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[below= 1.25cm of topl,align=left,anchor=west](0l){%
    ...
    %%% OPPOSITIONS
    \draw[<->,dashed, bend left = 30] (0t) to node[midway,font=\footnotesize,above]{±voice} (0d);
    \draw[<->,dashed, bend left = 30] (2xt) to node[midway,font=\footnotesize,above]{±length} (2lt);
    \draw[<->,dashed, bend left = 30] (2lt) to node[midway,font=\footnotesize,above]{±voice\vphantom{g}} (2xd);
    \draw[<->,dashed, bend left = 30] (3xt) to node[midway,font=\footnotesize,above]{±voice\vphantom{g}} (3lt);
    \draw[<->,dashed, bend left = 30] (3lt) to node[midway,font=\footnotesize,above]{±length} (3xd);
    %%% EXPLANATION
    \node[%
    below=8.5cm of topl,
    font=\footnotesize,
...
\tqt{As a result of phonemic lenition, \textit{*p, *t, *k} became voiced short \textit{*b, *d, *g}. These were constrasted with the reflexes of the unlenited voiced stops, which were now phonemically long (and, presumably, tense), merging with the rare old geminate voiced stops: \textit{*bː, *dː, *gː}. In word-initial position, the contrast between short and long voiced stops is maintained […] in MoB dialects. % The unlenited voiceless stops were undoubtedly long too, \textit{*pː, *tː, *kː}, but they did not contrast with short voiceless stops. MoW evidence shows that these were probably strongly aspirated […].
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    ...
    \node[descr,font=\footnotesize] at (0,-1) {%
      Wavy lines represent vibration of vocal cords.\\
...
Chapter~\ref{cha:prov-mwbe-y} analyses sandhi patterns of stops in the Poetry of the Princes, which was composed roughly from the eleventh century to the thirteenth century. The sandhi patterns found in this corpus  may be studied through the cynghanedd, the system of consonant harmony in poetry which is still in use today. The present-day cynghanedd  contains some provection rules. Within these rules, the pronunciation of a consonant may change depending on the preceding consonant, and may as a result alliterate with another consonant than itself. Chapter~\ref{cha:prov-mwbe-y} compares how these modern rules differ from the rules of the Poets of the Princes. It finds some differences between the two periods, and proposes a stop phonology within which the earlier rules may be understood. This stop phonology posits both length and aspiration as variables in distinguishing \xT, \lT, and \xD.
