...
\begin{table}[h]
  ...
  \begin{tabular}{lllp{0.25\linewidth}}
    ...
    \ow{bri\al{c}er, bri\al{c}eriauc} & \mow{briger, brigerog} & \ow{c} for /ɡ/ & \gpie{*bʰreǵ} \\
...
\mwcc[]{\acrshort{mc} 8 b.a.}{\ow{or\al{c}ueeticc cors} \textup{gl.~}\lat{ex papyro textili}}{out of the woven reed.}
...
\label{ptkwithbdg}
The word \ow{hendat} mentioned in the opening of this chapter provides an example of the type of spelling discussed in this section. Here, lenition is not only written, but is in fact written using the same consonant as its unlenited voiced counterpart \ow{d}. This may be interpreted as evidence that word-medial \lT\ had merged with \xD.  Alternately, orthographical representation with \ow{nt} was not possible here, even if the writer acknowledged the stop as being originally voiceless: in the \gls{ow} period, word-medial /nt/ became /nh/, with perhaps /n̥\textsuperscript{t}/ as an intermediary. \Gls{ow} orthography may have lagged behind this phonological change here, leading scribes to associate the cluster \ow{nt} with /nh/ or /n̥̤\textsuperscript{t}/, neither of which should be read in \ow{hendat}\footnote{Note, however, that \gow{Hanther} `half' and \ow{pimphet} `fifth' are attested, showing that the clusters \ow{nth} and \ow{mph} were used also for clusters undergoing the sound shift of \textit{NT -> Nh} word-medially.}. Choosing \ow{nd} would consequently be the only spelling left. However, the case of \ow{hendat} is not unique in its complexities. Similar consonant clusters containing a \gls{rescon} and a stop tend to write pre-\gls{apoc} voiceless stops with \ow{b, d, g}. Table~\ref{owvoicedstops} lists all words containing a pre-\gls{apoc} lenited voiceless stop that is represented by a voiced stop.
