...
\begin{sidewaysfigure}[h]
  ...
  \begin{tikzpicture}[
    ->,
    align=left,
    condit/.style={font=\small,above,near end, sloped}]
    \node(pb){Proto-Brittonnic};
    ...
    \node(t4ri) at (spir|-t3r){{id.}};
    \node(t4l) at (spir|-t3l){{id.}};
    \node(t5ri) at (merge|-t4ri){{/pʰ/}\\{/tʰ/}\\{/kʰ/}};
    \node(t5rs) at (merge|-t4rs){{id.}};
    %% Voiced stops
    \node[below=75mm of pb](d1){/p/\\/t/\\/k/};
    \node[left =of d1](d){\gls{D}\asp[-]};
    ...
    \node at (spir|-d3r)(d4r){{id.}};
    \node at (spir|-d3l)(d4l){{id.}};
    \node at (merge|-d4l)(d5l){{id.}};
    %% Merged stops
    \node  at ($(t4l)!0.5!(d4r)$)(td4){/p/\\/t/\\/k/};
    ...
    \draw (d2l)to node[above] {\lng[-]}(d3l);
    \draw[dashed] (t3r) to node[condit] {/¬\#\_}(t4rs);
    \draw[dashed] (t3r) to node[condit] {/\#\_}(t4ri);
    \draw(t4ri)--(t5ri);
    \draw[dashed] (t3l) to node[condit] {/\#\_}(t4l);
    \draw(t4l)--(td5);
    \draw(d4r)--(td5);
    \draw[dashed] (d3r) to node[condit] {/\#\_}(d4r);
    \draw (d3l)--(d4l)--(d5l);
    \draw (td4)--(td5);
    \draw[dashed] (d3r)to node[condit] {/¬\#\_}(td4);
    \draw[dashed] (t3l)to node[condit] {/¬\#\_}(td4);
    \draw(t4rs)--(t5rs);
  \end{tikzpicture}
  \caption{Summary of posited phonological developments}
