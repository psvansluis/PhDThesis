...
\end{mwl}
The word \mw[]{gyn} is erroneously removed in \gls{ll27}. Its removal is probably the result of an eyeskip, because \mw[]{gy-} is found twice in a row. Alternatively, equatives are sometimes formed using \mw[]{cy(f)-}, \eg \mw[of the same color]{kyfliw}~\autocite[§~41]{evans_grammar_1964}, and the scribe of \gls{ll27} took \mw[]{gyffredinet} as such a word.  In any case, we do not know now whether \gls{ll27} would have had \mw[]{cyn} or \mw[]{gyn} otherwise, and the lenited words found are lenited due to different causes, so this instance cannot be counted.
...
\begin{table}[h]
  ...
  \caption{Percentual representation of lenition of voiceless stops in \mw[]{Buchedd Dewi}}
...
\begin{figure}[h]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      width=.5\linewidth,
      height=.5\linewidth,
      xlabel={Number of manuscripts representing lenition},
...
\begin{figure}[h]
  ...
  \begin{subfigure}[b]{0.5\linewidth}
    ...
    \label{sfig:illconv}
  \end{subfigure}
  \caption{Stemmatic relationship and convergent evolution visualised.}
