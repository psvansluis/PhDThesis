...
Such a formulation must be borne out by the evidence. Based on orthographical evidence, \gls{mw} word segmentation agrees largely, but not completely, with where \gls{mow} orthography puts spaces between words. Still, exceptions exist, \eg \mw[together]{i gyd} is written as  one word in \gls{mw}.
...
The fact that we nowadays write a space between conjunctions, articles, and adverbs, \etc and the following word is not grounded linguistically, and other languages may do things differently, \eg \glat[and men]{virum-que}, Danish \textit{stol-en} `the chair'.
...
Rather, it may be considered a specific type of internal sandhi where a word takes one of these prefixes\todo{This is basically the same as jtk saying spirantization is only found following clitics.}.
